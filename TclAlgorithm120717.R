...
#Remove low width
  ...
  #FCS = log10(filedata$Peak9) 
  filedatals$sumCh =  apply(filedatals[,1:8],1,sum)
  #filedatals$sumCh <- log10(filedatals$sumCh)
  
  #with(filedatals,plot(sumCh,log10(FCS),pch  = 19,cex = 0.2))
  ...
  hsumch$mids = fr$x
  hsumch$counts = fr$y
  #fmax_hsumch = maxiNu(hsumch$mids,hsumch$counts,TRUE,plotpath,"sumCh",FALSE,30,10)
  fmax_hsumch = maxiNu(hsumch$mids,hsumch$counts,FALSE,plotpath,"sumCh",FALSE,30,10)
  ...
  if(length(fmax_hsumch$maxl) > 0 ){
    ...
    # }else{
    #     
    #   bondaryv1 = bondaryv - 0.2 * ( fmax_hsumch$mins$x[2] - fmax_hsumch$maxl$x[1] )   
    # }
  ...
  #fmax_hsumch  = findmax1(hsumch$mids, hsumch$counts,10,10,5,20,main = CarName)
  #Try New max 210617DD
  ...
  #sel_sumCh_atmin  = sumCh > bondaryv
  ...
  #fmax_hsumch = maxiNu(hsumch$mids,hsumch$counts,TRUE,plotpath,"sumCh",FALSE,30,10)
  fmax_h_Area1_fcs = maxiNu(h_Area1_fcs$mids,h_Area1_fcs$counts,FALSE,plotpath,"h_Area1_fcs",TRUE,30,10)
  ...
  #df = cbind(A1 = filedatal$Area1,FCS = filedatal$FCS)
  #df = scale(df)
  #cl = kmeans(df,centers = rbind(c(0,0),c(3,3)),nstart = 2)
  ...
  h_Area6_sel45  = hist(filedatal_sel45$Area6,200,plot = T )
  fr = filter1(bf,5,h_Area6_sel45$mids, h_Area6_sel45$counts)
  h_Area6_sel45$mids = fr$x
  h_Area6_sel45$counts = fr$y
  #fmax_hsumch = maxiNu(hsumch$mids,hsumch$counts,TRUE,plotpath,"sumCh",FALSE,30,10)
  fmax_h_Area6_sel45 = maxiNu(h_Area6_sel45$mids,h_Area6_sel45$counts,TRUE,plotpath,"Cd3 h_Area6",TRUE,30,10)
...
if(FALSE){
  ...
  fmax_h1 = maxiNu(h_FCS_Area1$mids,h_FCS_Area1$counts,TRUE,plotpath,"h1",FALSE,30,5)
  ...
  # maxind = which(h_FCS_Area1$counts == max(h_FCS_Area1$counts))
  # selmaxind = FCS_Area1 < h_FCS_Area1$mids[maxind]
  # h_FCS_Area1  = hist(FCS_Area1[selmaxind],100)
  # maxind = which(h_FCS_Area1$counts == max(h_FCS_Area1$counts))
  # selmaxind = FCS_Area1 < h_FCS_Area1$mids[maxind]
  # h_FCS_Area1  = hist(FCS_Area1[selmaxind],100)
  # df = data.frame(x = h_FCS_Area1$mids, y = h_FCS_Area1$counts)
  # le = length(df$x)
  # plot(df)
  # bu = NULL
  # 
  ...
  # plot(df$x[2:(le-2)],bu)
  # 
  # cut95  = 0.9 * max(bu,na.rm = T)
  # maxlocation = which(max(bu) == bu)
  # selcut95  = min(maxlocation + which( bu[maxlocation:length(bu)] < cut95))
  # xcut = df$x[ selcut95 ]
  # 
  #plot(df)
  #abline(v = xcut,col = 4 ,lwd = 2 ,lty =2)
  # if(PLOTS){
  #   png(filename= paste0(plotpath,"Hirsh Algorithm Result.png")) 
  #   with(filedatal,plot(Area1,10^FCS,col = 1 + (FCS_Area1 > xcut), pch = 19,cex = 0.2))
  #   dev.off()
  # }
  ...
  h6 = with(filedatal[sel45Hirsh,],hist(Area6,100))
  fr = filter1(bf,5,h6$mids, h6$counts)
  h6$mids = fr$x
  h6$counts = fr$y
  fmax_h6 = maxiNu(h6$mids,h6$counts,FALSE,plotpath,"h6",FALSE,30,10)
  ...
  h1 = with(filedatal[sel45Hirsh & sel6, ], hist(Area1,100) )
  fmax_h1 = maxiNu(h1$mids,h1$counts,FALSE,plotpath,"h1",FALSE,30,5)
...
#----------------------------
  ...
  # h6 = with(filedatal[sel45Hirsh,],plot(Area6,Area1,pch  =19,cex =0.2))
  # 
  # xout = xcut
  # h6 = with(filedatal[sel45Hirsh,],hist(Area6,100,plot = T))
  # fr = filter1(bf,5,h6$mids, h6$counts)
  # h6$mids = fr$x
  # h6$counts = fr$y
  # fmax_h6 = maxiNu(h6$mids,h6$counts,FALSE,plotpath,"h6",FALSE,30,5)
  # 
  ...
  #   }else{
  #     
  #     cut6 = quantile(filedatal[sel45Hirsh,]$Area6,2e-2)
  #   }
  #   
  #   sel6Hirsh = ( filedatal$Area6 > cut6 )
  #   cut6_1 = with(filedatal[sel6Hirsh,],quantile(Area1,0.05))
  # }
  # 
  # if(PLOTS) {
  #   
  ...
  #   sel45loop  = FCS_Area1 > xloop
  #   bu = c(bu,with(filedatal[!sel6Hirsh & sel45loop,],length(Area1[Area1 < cut6_1])/length(Area1[Area1 > cut6_1])))
  #   xu = c(xu ,xloop)
  # }
  # 
  # 
  # if(sum( bu > 0.05 ) > 0 ){
  #   xout = xu[min(which(bu < ( min(bu) + 0.05 *(max(bu) - min(bu)))))]
  # }
  # 
  # sel45Hirsh = FCS_Area1 > xout
  # 
  ...
  # if(PLOTS) {
  #   
  #   png(filename= paste0(plotpath,"Area1 vs Area6 Hirsh Algorithm",".png"))
  #   
  #  
  #   sel45s  = FCS_Area1 > (xout - 0.1)
  #   
  ...
  #   
  #   
  #   with(filedatal[sel45Hirsh,],points(Area6,Area1,pch =19 ,col = 1 ,cex =0.3,xlim = c(0,3),ylim = c(1,4)))
  #   abline(v = cut6,col = 2,lwd =2 ,lty  = 2 )
  ...
  # dee = 6
  # m1  = lm(y~stats::poly(x,dee),data = df)
  # predicted.intervals <- predict(m1,data.frame(x=df$x))
  # lines(df$x,predicted.intervals,"l",lwd =2 ,col =2)
  # 
  # plot(df$x[3:length(df$x)],diff(diff(predicted.intervals)))
  # 
  # u = (m1$coefficients[3:(dee+1)]*c(2,6,12,20))%*%t(cbind(1,df$x,df$x^2,df$x^3))
  # plot(1:length(u),u)
  # 
  #with(filedatal,plot(Area1,10^FCS,pch = 19,cex = 0.2 ))
  ...
  h6 = with(filedatal[sel45Luda,],hist(Area6,100,plot = T))
  fr = filter1(bf,5,h6$mids, h6$counts)
  h6$mids = fr$x
  h6$counts = fr$y
  fmax_h6 = maxiNu(h6$mids,h6$counts,TRUE,plotpath,"h6",FALSE,30,5)
  ...
  h_Area1_max = maxiNu(h_Area1$mids,h_Area1$counts,TRUE,plotpath,"h_Area1",FALSE,30,10)
  ...
  # else{
  # 
  #   filedatal45pl = read.csv(wrkingFilepath,header = T)
  #   re = (strsplit(wrkingFilepath,"/")[[1]])
  #   CarName = (strsplit(re[length(re)],".c")[[1]])[1]
  #   Cartnum = as.numeric((strsplit(CarName, "[^[:digit:]]")[[1]])[2])
  # 
  # }
...
#cut the CD4 tail by using parllel projection
#lm68 = with(filedatal45pl,lm(Area8 ~ Area6 )) # FirstVer
...
h_vpArea6Area8_max = maxiNu(h_vpArea6Area8$mids,h_vpArea6Area8$counts,TRUE,plotpath,"h_vpArea6Area8 SelPreDead",FALSE,30,10)
...
# if(PLOTS){
#   png(filename= paste0(plotpath,"dead from SelPreDead",".png"))
#   with(filedatal45pl_predead,plot(Area6,Area8,pch =19,cex= 0.2,xlim = c(-2,3)))
...
if(PLOTS){
  ...
  #change new max 210617DD
  #png(filename= paste0(plotpath,"h_lm68hv",".png"))#plot12
...
h_lm68hv_max = maxiNu(h_lm68hv$mids,h_lm68hv$counts,TRUE,plotpath,"h_lm68hv Detect Deads",FALSE,30,10)

if( length(h_lm68hv_max$maxl) == 0 ){
  
  dev.off()
  h_lm68hv_max = maxiNu(h_lm68hv$mids,h_lm68hv$counts,TRUE,plotpath,"h_lm68hv Detect Deads second",FALSE,20,10)
...
if( length(h_lm68hv_max$maxl) != 0 ){
  ...
  if(length(h_lm68hv_max$maxl$x) == 1){
    ...
    #cut0.02ind = max( 1 , SimpleLeftCut(h_lm68hv_max_right,h_lm68hv,1e-1) - 1)
...
if(FALSE){
  ...
  h_lm68hp_max = maxiNu(h_hist6$mids,h_hist6$counts,TRUE,plotpath,"h_hist6_CD45_Area6 CD3 Detection",FALSE,30,10)
...
h_lm68hv_max = maxiNu(h_vps34$mids,h_vps34$counts,TRUE,plotpath,"h_vps34 Cut Tail",FALSE,30,10)
if( length(h_lm68hv_max$maxl) == 0 ){
   
   dev.off()
   h_lm68hv_max = maxiNu(h_vps34$mids,h_vps34$counts,TRUE,plotpath,"maxNe_h_vps34",FALSE,20,10)
}
...
# if(PLOTS){
#   
#   leMax = length(h_34max$maxl$indx)
#   
#   if( leMax > 2 ){
#     
#       bounary8 = h_34max$mins$x[leMax]
#       abline(v = bounary8,col = 1,lwd = 2,lty = 2 )
#   
#       bounary4 = h_34max$mins$x[2]
#       abline(v = bounary4,col = 1,lwd = 2,lty = 2 )
#       
#       dev.off()
#   }
#     
...
#     abline(v = bounary4,col = 1,lwd = 2,lty = 2 )
#     
#     sel8  =  h34$mids > bounary8
#     sel4  =  h34$mids < bounary4
#     
#     me8 = mean(h34$mids[sel8])
#     sd8 = sd(h34$mids[sel8])
#     me4 = mean(h34$mids[sel4])
#     sd4 = sd(h34$mids[sel4])
#     
#     t8  = abs(h34$mids - me8)/sd8
#     t4  = abs(h34$mids - me4)/sd4
#     
#     #plot(h34$mids,t8 - t4,"l",col =2)
#     #points(h34$mids,t4,"l",col =3)
#     
#     
#     dev.off()
#   }
...
if(!bUseRatio ) { 
  ...
  #undebug(maxiNu)
  h_vvs34woTailvvs_max = maxiNu(h_vvs34woTail$mids,h_vvs34woTail$counts,TRUE,plotpath,"h_vvs34woTail Detect CD4 CD8",FALSE,10,6)
...
if(!bType8) 
{
  ...
  h_vvslmonly4_max = maxiNu(h_vvslmonly4$mids,h_vvslmonly4$counts,TRUE,plotpath,"h_vvslmonly4 Detection DN",FALSE,10,10)
...
#Change Cd4 DN if if low CD3
# cd4CH3qu02 = quantile( filedatalCD3$Area4[selType40r8],0.02 )
# 
# if (  cd4CH3qu02  > median(filedatalCD3$Area3[!selType40r8]) ){
#   
#   sel_cd4CH3qu02  = cd4CH3qu25  >  filedatalCD3$Area3[!selType40r8]
#   
#   sel_doubleNegtoveBoundaryIndForAll = sel_doubleNegtoveBoundaryIndForAll | sel_cd4CH3qu02
#   
# }
